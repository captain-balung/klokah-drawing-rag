...
- 🔄 回滾演練 → Vercel ✅：`vercel rollback 705b0djqv`（退）→ `vercel promote 457ibcbto`（切回），`vercel inspect` 證實別名指向移動並復原、Playwright 證功能完好（CLI，2026-05-31）。Render ⏸️→**已解鎖**：Phase 5 上線後後端已有 ≥2 個 deploy 版本（`f970d48` → `35553d6` → `2d0502e` → `e0770f3`），可互退；待人類擇期演練再補 VERIFY
