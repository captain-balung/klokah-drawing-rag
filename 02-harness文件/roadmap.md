...
- **整體**：約 60%
- **當前 Phase**：Phase 3（前端介面）進行中——React 對話畫面已串接後端、多輪實測通過
- **當前焦點**：繪本瀏覽畫面（F-04）與閱讀畫面（F-02，逐頁圖片 + 族語切換 + 音檔）
- **近期完成**：Phase 2 完成——後端 API（F-01~F-04/F-07/F-08）全綠、prompt caching 命中、幻覺防護、成本監控、ruff/pytest 接線（`ruff check` 綠、`pytest` 預設免費案例綠、live 案例標記）
...
```
Phase 3（前端介面）→ 子任務「繪本瀏覽畫面 F-04」→ 葉節點「依級別/語法/族別過濾繪本卡片」
```

- 預計完成時間：中期
- 信心程度：中（對話畫面已跑通；瀏覽/閱讀畫面為新工作）
- 上一個完成：對話畫面串接後端 ✅（多輪實測，含修掉多輪 400 bug）
- 設計註記：族語逐字忠實以資料層為準（前端閱讀畫面直接渲染 JSON）；字型須涵蓋族語變音符號（ʉ ʔ ə）
